...
const productValidation = [
    ...
    body('stock').if((value, { req }) => !value).isInt({ min: 0 }).withMessage('El stock debe ser entero no negativo.').trim().escape(),
    body('precioProdAlta').if((value, { req }) => !value).isInt({ min: 0 }).withMessage('El stock debe ser entero no negativo.').trim().escape(),
    body('descuentoProdAlta').if((value, { req }) => !value).isInt({ min: 0 }).withMessage('El stock debe ser entero no negativo.').trim().escape(),
